...
#Inputs
routes_master = readRDS("../pct-lsoa/Data/03_Intermediate/routes/rf_nat_4plus_fix.Rds")   #CHANGE ME - input rf_nat_all.Rds
flow_data = read.csv("E:/OneDrive - University of Leeds/Cycling Big Data/LSOA/flow_results_nat_round_170121.csv") #CHANGE ME - input flow_results_nat_round_170121.csv
outfld <- "C:/results/test/" #CHANGE ME - where results are saved 
...
routes_master@data = routes_master@data[,c("ID","ebike_slc_r")]  #CHANGE ME #Change for different scenarios
remove(flow_data)
routes_master = routes_master[routes_master$ebike_slc_r >0,] #CHANGE ME #Change for different scenarios
nrow(routes_master)
...
for(i in tab$grid){
  ...
  if(nrow(polys) <= limit){
    ...
    for(l in 1:ceiling(nrow(polys)/limit)){
      ...
      rsum <- stackApply(rs, 1, sum)
      #print(paste0("Added ",Sys.time()))
      writeRaster(rsum,filename = paste0(outfld,"grd-",i,"-",k,".tif"), format ="GTiff")
      #print(paste0("Written ",Sys.time()))
      removeTmpFiles(h = 1)
      remove(rs,rsum)
