...
#Inputs
routes_3l = readRDS("../pct-lsoa/Data/03_Intermediate/routes/rf_nat_less3p_fix.Rds")   #CHANGE ME
routes_4p = readRDS("../pct-lsoa/Data/03_Intermediate/routes/rf_nat_4plus_fix.Rds")
...
# Join two routes files togther
# From http://stackoverflow.com/questions/32509536/shape-file-prisecroads-abutting-two-states
nAL <- length(routes_3l@lines)
nGA <- length(routes_4p@lines)
nAL2 <- max(as.integer(rownames(routes_3l@data)))
for (i in 1:nGA) routes_4p@lines[[i]]@ID=as.character(nAL2+i)
rownames(routes_4p@data) <- 1:nGA + nAL2
...
routes_master <- spRbind(routes_4p,routes_3l)
saveRDS(routes_master, "../pct-lsoa/Data/03_Intermediate/routes/rf_nat_all.Rds")
remove(routes_3l,routes_4p)
#routes_master <- rbind(routes_4p, routes_3l, makeUniqueIDs = TRUE)
#routes_master <- rbind.Spatialroutes_4pLinesDataFrame(routes_4p,routes_3l, makeUniqueIDs = TRUE)
