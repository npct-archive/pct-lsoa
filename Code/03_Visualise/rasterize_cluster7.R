...
#Inputs
infld <- ""
inname <- "ebike_slc_r-" #Which scenario do you want to do gendereq_slc_r- govtarget_slc_r-  dutch_slc_r- ebike_slc_r-
outfld <- "results/ebikel3/"
clusterNo <- 1
start_grd <- 1 #Where to start, for resuming after a failure, 1 = start a beginning

#Parameters
#rasterOptions(tmpdir = paste0("C:/RasterTemp/",outfld,"cl",clusterNo)) #FOR PC ONLY coment out on cluster
...
#Change the temp folders - for the LEEDS CLUSTER
if (!isTRUE(file.info(paste0(outfld,"cluster-",clusterNo,"/temp"))$isdir)) dir.create(paste0(outfld,"cluster-",clusterNo,"/temp"), recursive=TRUE)
if (!isTRUE(file.info(paste0(outfld,"cluster-",clusterNo,"/rtemp"))$isdir)) dir.create(paste0(outfld,"cluster-",clusterNo,"/rtemp"), recursive=TRUE)
Sys.setenv(TMP = paste0(getwd(),"/",outfld,"cluster-",clusterNo,"/temp"))
rasterOptions(tmpdir = paste0(getwd(),"/",outfld,"cluster-",clusterNo,"/rtemp"))
...
for(i in tab$grid){
  ...
  if(nrow(polys) <= limit){
    ...
    #Save Raster and clean up
    writeRaster(rsum,filename = paste0(outfld,"cluster-",clusterNo,"/grd-",i,".tif"), format ="GTiff", datatype = "INT2U")
    removeTmpFiles(h = 1)
    remove(rsum)
    gc(verbose = FALSE)
    ...
    for(l in 1:ceiling(nrow(polys)/limit)){
      ...
      rsum <- calc(rs, sum)
      remove(rs)
      writeRaster(rsum,filename = paste0(outfld,"cluster-",clusterNo,"/parts/grd-",i,"-",l,".tif"), format ="GTiff", datatype = "INT2U")
      remove(rsum)
      gc(verbose = FALSE)
    ...
    rsum <- calc(rs, sum)
    remove(rs)
    writeRaster(rsum,filename = paste0(outfld,"cluster-",clusterNo,"/grd-",i,"-all.tif"), format ="GTiff", datatype = "INT2U")
    remove(rsum)
